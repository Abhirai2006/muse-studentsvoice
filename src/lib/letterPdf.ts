...
export async function generateLetterPDF(letter: PendingLetter, recipients: Recipient[]) {
  ...
  doc.setFontSize(10);
  doc.text(
    "Mysore University School of Engineering — Peer-verified student grievance",
    pageWidth / 2,
    y,
    { align: "center" },
  );
  ...
  y += 14;
  doc.text(`Reference: SV/${ref}`, margin, y);
  y += 14;
  doc.text(`Submitted: ${new Date(letter.createdAt).toLocaleString("en-IN")}`, margin, y);
  y += 14;
  doc.text(
  ...
  if (recipients.length > 0) {
    ...
    for (const r of recipients) {
      doc.text(`${r.name} (${r.role}) — ${r.email}`, margin, y);
      y += 14;
    }
    y += 10;
  }
  ...
  // Complaint body
  doc.setFont("times", "bold");
  doc.text("Complaint (verbatim):", margin, y);
  y += 16;
  ...
  doc.setFont("times", "normal");
  const closing =
    "The author's identity is withheld to protect the student, but they are a verified student of the institution whose USN was authenticated at the time of posting. The complaint has been reviewed by the student body and passed the platform's verification threshold.";
  const closingLines = doc.splitTextToSize(closing, contentWidth);
  if (y + closingLines.length * 14 > 780) {
    doc.addPage();
    y = margin;
  }
  ...
  y += 14;
  doc.setFont("times", "bold");
  doc.text("The MUSE Student Body", margin, y);
  y += 14;
  doc.setFont("times", "italic");
  doc.setFontSize(9);
